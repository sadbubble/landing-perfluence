 ...
 *
 * ЕДИНСТВЕННЫЙ источник правды: этот файл импортирует и Edge Function
 * (Deno), и фронтенд (src/lib/phone.ts). Не дублировать логику — если
 * клиент и сервер разойдутся в трактовке номера, поедет дедупликация
 * и связь с клиентом.
 *
 * Зависимостей нет намеренно: файл должен исполняться и в Deno, и в браузере.
 */
