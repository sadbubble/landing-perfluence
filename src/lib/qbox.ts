...
/**
 * Ключей тарифа два, и шлём мы оба с одним значением.
 *
 * ТЗ противоречит само себе: в одних местах ключ назван `Product_name`,
 * в других `tariff`. Проверка на боевой форме показала, что Qbox принимает
 * оба — значение легло и в поле «ПРодукт», и в поле «тариф». Лишний параметр
 * ничего не стоит, зато ссылка переживёт смену маппинга на их стороне.
 */
const PRODUCT_KEYS = ["Product_name", "tariff"] as const;
