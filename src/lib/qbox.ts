...
/**
 * НАЗВАНИЯ ТАРИФОВ, УХОДЯЩИЕ В QBOX.
 *
 * Значение едет в поле «ПРодукт» формы и хранится там ТЕКСТОМ — так
 * подтвердил заказчик после боевого теста. Отдельный каталог продуктов в
 * Qbox есть, но заполнять его не нужно и, по их словам, нельзя.
 *
 * Отсюда системная пометка `[unresolved_products]` в комментарии к лиду:
 * Qbox ищет совпадение в каталоге и не находит. На саму заявку это не
 * влияет — на вопрос, что означает пометка, в Qbox ответили, что не знают.
 * Проверено настоящим лидом: «ПРодукт: Bereket 4 SIM (без контракта)»
 * записался целиком и правильно.
 *
 * Значит эти строки — не технические коды, а ТЕКСТ, который читают глазами
 * менеджеры продаж и по которому в конце месяца считают выработку дилеров.
 * Поэтому они на русском и содержат режим контракта с числом SIM: без них
 * «Bereket» ничего не говорит о цене.
 *
 * Названия взяты из таблицы ТЗ п.4. Маркетинг по слайду 14 «Инструкции по
 * запуску лидогенерации» готовит свой финальный список — если он придёт и
 * будет отличаться, меняются ровно эти девять строк и больше ничего.
 *
 ...
 *
 * Возвращает null, если сочетания нет в таблице: лучше отправить заявку без
 * тарифа, чем с выдуманным названием. Пустое поле менеджер уточнит звонком,
 * а неверное примет за правду и внесёт в отчёт по дилерам как есть.
 */
